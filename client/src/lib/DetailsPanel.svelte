...
<div class="flex h-full flex-col">
  {#if !sel}
    <div class="flex h-full flex-col items-center justify-center gap-2 p-6 text-center text-sm text-muted">
      ...
      <p>Select a class, property, or instance to inspect it.</p>
      <p class="text-xs">Click nodes in the graph or items in the sidebar.</p>
    </div>
  {:else}
    ...
    <div class="flex-1 overflow-y-auto p-3 text-sm">
      ...
      {:else if ind}
        ...
        {#if ind.assertions.length}
          {@render section('Property values')}
          <ul class="space-y-1">
